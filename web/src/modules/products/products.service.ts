...
export class ProductsService {
      ...
      for (const shop of shops) {
        ...
        if (products.ok) {
          ...
          if (!productsInDB.length) {
            ...
            const newProducts = reworkedProducts.map((product) => {
              return {
                type: 'new',
                product: product,
              }
            ...
            await this.updateProductsInStore(newProducts, shop)
          ...
          for (const product of reworkedProducts) {
            ...
            for (const key in product) {
              ...
              if (key === 'properties') {
                ...
                if (!product[key] || !product[key].length) {
                  continue;
                }
  ...
  async updateProductsInStore(changedProducts: ChangedProduct[], store: any) {
    try {
      console.log('---------------- updateProductsInStore ----------------')
        ...
        if (product.type === 'new') {
          ...
          for (const key in productData) {
            ...
            switch (key) {
              ...
              case 'properties':
                for (const property in productData.properties) {
                  if (property === 'category') {
                    newProduct.product_type = productData.properties[property];
                    continue;
                  }

                  if (property === 'tags') {
                    newProduct.tags = productData.properties[property];
                    continue;
                  }

                  metafields[property] = productData.properties[property];
                }
          ...
          if (productReference) {
            ...
            const shopifyProduct = await shopify.api.rest.Product.delete({
              session: session,
